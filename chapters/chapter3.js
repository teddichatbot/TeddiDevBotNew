...
var playV4video = (currentMaster, currentBranch)=>{
    var URL = 'https://teddibucketdev.blob.core.windows.net/videoscontainer/V4 Amy\'s video on Baby sleep.mp4';
...
var playV5video = (currentMaster, currentBranch)=>{
    var URL = 'https://teddibucketdev.blob.core.windows.net/videoscontainer/V5 Lullaby Trust sleep position video.mp4';
    let url_list = '[{"English":"'+URL+'"}]'
    botReply = ''
    botReply += '#&@#{"selectVideo" : '+url_list+' }'
    // await turnContext.sendActivity(botReply);
...
var playV6video = (currentMaster, currentBranch)=>{
    var URL = 'https://teddibucketdev.blob.core.windows.net/videoscontainer/V6 co-sleeping.mp4';
    let url_list = '[{"English":"'+URL+'"}]'
    botReply = ''
    botReply += '#&@#{"selectVideo" : '+url_list+' }'
    // await turnContext.sendActivity(botReply);
...
var obj = {
    chatJson: async(chapterType,userSession,turnContext) => { 
        ...
        if(userSession[chapterType].mainMaster == ''){ //check givingHealth branch
            ...
            if(userMsg == 'play the lullaby trust sleep position video'){  
                return playV5video(mainMaster, mainBranch);
            }else if(userMsg == 'play the co sleeping video'){
                return playV6video(mainMaster, mainBranch);
            }else if(userMsg == "play amy's video on baby sleep" || userMsg == "play amy’s video on baby sleep"){
                return playV4video(mainMaster, mainBranch);
            }else{
